...
Codex is installed with `curl -fsSL https://chatgpt.com/codex/install.sh | CODEX_NON_INTERACTIVE=1 sh` as the admin user. The same command is rerun by `vps-agent-cli-update.timer` every two days. Post-setup auth uses `codex login` through `vps-agent-auth`.
